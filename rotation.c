...
void perform_rotation(SDL_Renderer* renderer, double angle_rotation,SDL_Texture* texture, int w, int h)
{
  ...
  SDL_RenderCopyEx(renderer, texture, NULL, &position, angle_rotation, &center, flip);
...
void event_loop(SDL_Renderer* renderer, SDL_Texture* texture, SDL_Surface * surface, int w, int h)
{
    SDL_Event event;
    double angle = 0;
	...
	case SDL_KEYDOWN:
	  ...
	  if (angle>=360)
	    angle-=360;
	  perform_angle(renderer, texture, angle,surface, w,h);
	  break;
...
int main(int argc, char** argv)
{
    ...
    event_loop(renderer,texture,surface, w, h);
